...
Follow the steps below to configure RabbitMQ for the `p48-beamline` namespace.
...
```bash
$ # module load k8s-p48
$ head -c 500 /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 16 | head -n 1 | kubeseal --raw --namespace p48-beamline --name rabbitmq-secrets
```
...
`p48-rabbitmq-daq.diamond.ac.uk -> Static IP` this mapping allows processes outside of the namespace e.g. Analysis to access data from rabbitmq.
